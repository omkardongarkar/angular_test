...
export class CustomerSatisfactionComponent implements AfterViewInit {
  ...
  @ViewChild('customerSatisfactionChart') customerSatisfactionChart!: ElementRef;
  chart!: Chart;
  ...
  initializecustomerSatisfactionChart(): void {
    this.chart = new Chart(this.customerSatisfactionChart.nativeElement, {
  ...
  getDataForAgent(agentName: string): number[] {
    const dataMap: { [key: string]: number[] } = {
      'Product Support Agent': [3, 2, 2, 1, 1, 2, 3],
      'IT Support Agent': [2, 2, 3, 4, 3, 2, 2],
      'Billing Support Agent': [3, 2, 3, 3, 2, 2, 2],
      'Developer Support Agent': [3, 3, 2, 3, 3, 2, 2],
      'Sales Support Agent': [3, 4, 4, 3, 3, 3, 2]
    };
